...
def test_stdin_batch():
    "Test that vtysh -s executes commands piped through stdin"
    ...
    step("pipe a batch of show commands into vtysh -s")
...
def test_stdin_config():
    "Test that configuration commands piped into vtysh -s are applied"
    ...
    step("configure a static route through vtysh -s")
    r1.cmd_raises(
...
def test_stdin_persistent_pipe():
    "Test that vtysh -s waits for further commands until stdin is closed"
    ...
    fifo = "/tmp/vtysh_stdin_r1.fifo"
    outf = "/tmp/vtysh_stdin_r1.out"
    flag = "/tmp/vtysh_stdin_r1.flag"

    step("start vtysh -s reading from a fifo")
    r1.cmd_raises("rm -f {0} {1} {2} && mkfifo {0}".format(fifo, outf, flag))
    r1.cmd_raises(
        "( vtysh -s -E < {0} > {1} 2>&1; echo EXITCODE:$? >> {1} )"
...
def test_stdin_exit_codes():
    "Test vtysh -s exit codes and option conflicts"
    ...
    assert rc != 0, "vtysh -s did not exit non-zero on unknown command"
    assert (
        "FRRouting" not in output
    ), "vtysh -s kept executing after a failed command: \n{}".format(output)

    step("with -n a failing command must not affect the exit code")
    rc, _, _ = r1.net.cmd_status(
        "printf 'show garbage nonsense\\nshow version\\n' | vtysh -s -n", warn=False
    )
    assert rc == 0, "vtysh -s -n exited non-zero: {}".format(rc)

    step("-s must be rejected in combination with -c")
    rc, _, error = r1.net.cmd_status("vtysh -s -c 'show version'", warn=False)
    assert rc != 0, "vtysh accepted -s combined with -c"
